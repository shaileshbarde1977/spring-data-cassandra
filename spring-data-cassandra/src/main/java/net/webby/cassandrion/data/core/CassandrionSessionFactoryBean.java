...
import net.webby.cassandrion.core.cql.options.KeyspaceOptions;
import net.webby.cassandrion.core.cql.options.KeyspaceReplicationOptions;
import net.webby.cassandrion.core.cql.spec.KeyspaceOption;
import net.webby.cassandrion.core.cql.spec.KeyspaceOption.ReplicationOption;
import net.webby.cassandrion.data.config.KeyspaceAttributes;
import net.webby.cassandrion.data.config.TableAttributes;
...
public class CassandrionSessionFactoryBean implements FactoryBean<Session>, InitializingBean, DisposableBean,
		BeanClassLoaderAware, PersistenceExceptionTranslator {

	private static final Logger log = LoggerFactory.getLogger(CassandrionSessionFactoryBean.class);
	...
	public void afterPropertiesSet() throws ClassNotFoundException {
		...
		if (StringUtils.hasText(keyspace)) {
			...
			CassandrionAdminTemplate cassandraAdminTemplate = new CassandrionAdminTemplate(session, converter, keyspace);
	...
	private void createNewTable(CassandrionAdminTemplate cassandraAdminTemplate, String useTableName, Class<?> entityClass)
	...
	public void setConverter(CassandrionConverter converter) {
	...
	private static final CassandrionConverter getDefaultCassandraConverter() {
		MappingCassandrionConverter converter = new MappingCassandrionConverter(new CassandrionMappingContext());
